...
if __name__ == "__main__":
    ...
    if opt.ptq:
        if not opt.cali:
            ...
            if opt.use_aq:
                ...
                model.model.iter = 0
        else: # Batch size default 256 but oom -> 64
            ...
            # a_cali_data = tuple((cali_data[0].to('cuda'),cali_data[1].to('cuda')))

            # tmp = []
            # for i in range(0, opt.custom_steps, opt.interval_length): tmp.append([cali_data[0][i * 256: (i + 1) * 256], cali_data[1][i * 256: (i + 1) * 256]])
            # w_cali_data = [torch.cat([x[0].cuda() for x in tmp]), torch.cat([x[1].cuda() for x in tmp])]
            # import pdb;pdb.set_trace()
            # torch.save(a_cali_data,'./church_a_cali_data_256.pt')
            # torch.save(w_cali_data,'./church_w_cali_data_256.pt')
            # torch.cuda.empty_cache()
            logger.info("Church Calibration data generated.")

            a_cali_data=torch.load('./church_a_cali_data_256.pt')
            w_cali_data=torch.load('./church_w_cali_data_256.pt')
            # a_cali_data=torch.load('./bedroom_a_cali_data.pt')
            # w_cali_data=torch.load('./bedroom_w_cali_data.pt')
    ...
    logger.info("Set sampling seed to 2024")
